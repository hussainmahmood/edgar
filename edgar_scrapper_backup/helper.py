            ...
            if not key:
                ...
                while key != 'END_REPTOWN_RELATION':
                    ...
                    if key == 'DIRECTOR':
                        ...
                        ZSECFORM3_REPTER_dict['OFFICER'] = match.group('otherText').strip()
    ...
    while i < len(lines):
        ...
        if not key:
                ...
                if key == 'REPTOWN_CIK':
                    ...
                    if isinstance(index, int) and index >= 0:
                        while key != 'END_REPTOWN_RELATION':
                            ...
                            elif key == 'TEXT_OTHER': ZSECFORM4_REPTER_arr[index]['OFFICER'] = match.group('otherText').strip()
            ...
            if key == 'SIGN_NAME':
                ...
                print(comp_name)
                index = next((ix for (ix, d) in enumerate(ZSECFORM4_REPTER_arr) if d["REPT_CONFDNAME"].lower() == comp_name.lower()), None)
                if isinstance(index, int) and index >= 0:
                    if len(sign_names) == 1:
