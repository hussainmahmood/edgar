...
def to_bool(value):
    if value in ['1', 'true', 'True', 'TRUE', 't', 'T']:
        return '1'
    elif value in ['0', 'false', 'False', 'FALSE', 'f', 'F']:
        return '0'

    return value

def read_ciks(cik_file):
    return pd.read_csv(f"cik/{cik_file}", header=0, dtype=object, squeeze=True)

def create_directory(cik):
    if not os.path.exists(f"companies/{cik}"):
        os.makedirs(f"companies/{cik}")
        ledger = pd.DataFrame(columns=['FORM_TYPE', 'LINK', 'FILENAME', 'DOWNLOADED', 'UPLOADED'], dtype=object)
    else:
        try:
            ledger = pd.read_csv(f'companies/{cik}/ledger.csv', header=0, dtype=object)
...
def get_links(drv, cik, ledger, latest_by):
    next_button = True
    last = datetime.today()
    exception_raised = False
    error = ""
    while next_button and last >= latest_by:
        try:
            ...
            for tr in pret.find_all("tr"):
                ...
                if tds:
                    ...
                    filed_date = tds[3].get_text().strip()
                    last = datetime.fromisoformat(filed_date[:10])
                    if last < latest_by:
                        break
                    if link not in ledger.values:
                      filename = link.split('/')[-1]
                      ledger = ledger.append(pd.DataFrame([[form_type, link, filename, 'FALSE', 'FALSE']], columns=['FORM_TYPE', 'LINK', 'FILENAME', 'DOWNLOADED', 'UPLOADED'], dtype=object))
            try:
    ...
    return ledger, exception_raised, error

def download_links(cik, ledger, forms_required, drv):
    exception_raised = False
    error = ""
    for i, entry  in ledger.iterrows():
        if entry['DOWNLOADED'] == 'FALSE' and entry['FORM_TYPE'] in forms_required:
            try:
                drv.get(f"https://www.sec.gov{entry['LINK']}")
                body = drv.find_element_by_tag_name('body')
                pret = bs(body.get_attribute('innerHTML'), features='lxml')
                text = pret.find('pre')
                with open(f"companies/{cik}/{entry['FILENAME']}", 'w') as f:
...
def save_ledger(cik, ledger):
    ledger.to_csv(f"companies/{cik}/ledger.csv", index=False)
    return None

def create_db_instance():
    params = urllib.parse.quote_plus('Driver={ODBC Driver 17 for SQL Server};' 'Server=.;' 'Port=1433;'
                                     'Database=xbrl_forms;' 'UID=sa;' 'PWD=tag$tag12;')
...
def parse_and_upload_form3(session, cik, filename):
    ...
    while i < len(lines):
        key, match = parse_line_form3(lines[i], func_time)
        if not key:
            i += 1
            continue
            ...
            ZSECFORM3_REPTER_dict['DATE_REPORT'] = match.group('report_period').strip()
        elif key == 'DATE_FILED':
            ...
            while key != 'ISSUER' and key != 'END_SEC_HEADER':
                ...
                if key == 'CIK':
                    ...
                    while key != 'END_SEC_HEADER' and key != None and key != 'ISSUER':
                        key, match = parse_line_form3(lines[i], func_time)
                        if key == 'STREET1':
            ...
            ZSECFORM3_dict['SECTION_16'] = to_bool(match.group('section16').strip())
            ...
            while key != 'END_REPTOWN_RELATION':
                key, match = parse_line_form3(lines[i], func_time)
                if key == 'DIRECTOR':
                    ZSECFORM3_REPTER_dict['DIRECTOR'] = to_bool(match.group('is_director').strip())
                elif key == 'OFFICER':
                    ZSECFORM3_REPTER_dict['OFFICER'] = to_bool(match.group('is_officer').strip())
                elif key == 'TENPERCENTOWNER':
                    ZSECFORM3_REPTER_dict['TENPERCENTOWNER'] = to_bool(match.group('is_tenpercent').strip())
                elif key == 'OTHER':
                    ZSECFORM3_REPTER_dict['OTHER'] = to_bool(match.group('is_other').strip())
                elif key == 'TEXT_OTHER':
...
rx_4 = {
    ...
    'SEC_FILENO': re.compile(r'SEC FILE NUMBER:(?P<file_no>.*)'),
    'FILENAME': re.compile(r'<FILENAME>(?P<filename>.*)'),
    'FILEDESCRIP': re.compile(r'<DESCRIPTION>(?P<filedescrip>.*)'),
    'XML_VERSION': re.compile(r'<\?xml version=\"(?P<xml_version>.*)\"'),
    'DATE_REPORT': re.compile(r'<periodOfReport>(?P<date_report>.*)</periodOfReport>'),
    'SIGN_BLOCK': re.compile(r'<ownerSignature>'),
...
def parse_and_upload_form4(session, cik, filename):
    ...
    while i < len(lines):
        ...
        if not key:
        ...
        elif key == 'SECTION_16': ZSECFORM4_dict['SECTION_16'] = to_bool(match.group('section16').strip())
            ...
            while key != 'END_REPTOWN':
                ...
                if key == 'REPTOWN_CIK':
                    ...
                    if isinstance(index, int) and index >= 0:
                        while key != 'END_REPTOWN_RELATION':
                            key, match = parse_line_form4(func_time, lines[i].strip())
                            if key == 'DIRECTOR': ZSECFORM4_REPTER_arr[index]['DIRECTOR'] = to_bool(match.group('is_director').strip())
                            elif key == 'OFFICER': ZSECFORM4_REPTER_arr[index]['OFFICER'] = to_bool(match.group('is_officer').strip())
                            elif key == 'TENPERCENTOWNER': ZSECFORM4_REPTER_arr[index]['TENPERCENTOWNER'] = to_bool(match.group('is_tenpercent').strip())
                            elif key == 'OTHER': ZSECFORM4_REPTER_arr[index]['OTHER'] = to_bool(match.group('is_other').strip())
            ...
            while key != 'END_NDH':
                ...
                elif key == 'SWAP': ZSECFORM4_NDTAB_dict['SWAP'] = to_bool(match.group('swap').strip())
            ...
            while key != 'END_DH':
                ...
                elif key == 'SWAP': ZSECFORM4_DTAB_dict['SWAP'] = to_bool(match.group('swap').strip())
...
def parse_and_upload_form5(session, cik, filename):
    ...
    while i < len(lines):
        ...
        if not key:
        ...
        elif key == 'SECTION_16': ZSECFORM5_dict['SECTION_16'] = to_bool(match.group('section16').strip())

        elif key == 'FORM3': ZSECFORM5_dict['FORM3HOLDRPTD'] = to_bool(match.group('form3').strip())

        elif key == 'FORM4': ZSECFORM5_dict['FORM4TRANXRPTD'] = to_bool(match.group('form4').strip())
            ...
            ZSECFORM5_ISSUER_dict['ISSUER_SYMBOL'] = match.group('issuer_symbol').strip()
             ...
             while key != 'END_REPTOWN_RELATION':
                key, match = parse_line_form5(func_time, lines[i].strip())
                if key == 'DIRECTOR': ZSECFORM5_REPTER_dict['DIRECTOR'] = to_bool(match.group('is_director').strip())
                elif key == 'OFFICER': ZSECFORM5_REPTER_dict['OFFICER'] = to_bool(match.group('is_officer').strip())
                elif key == 'TENPERCENTOWNER': ZSECFORM5_REPTER_dict['TENPERCENTOWNER'] = to_bool(match.group('is_tenpercent').strip())
                elif key == 'OTHER': ZSECFORM5_REPTER_dict['OTHER'] = to_bool(match.group('is_other').strip())
                elif key == 'TEXT_OTHER': ZSECFORM5_REPTER_dict['TEXT_OTHER'] = match.group('otherText').strip()
                elif key == 'OFFICER_TITLE': ZSECFORM5_REPTER_dict['OFFICER_TITLE'] = to_bool(match.group('off_title').strip())
                i += 1
            ...
            while key != 'END_NDH':
                ...
                if key == 'SECURITY_TYPETITLE':
                    ...
                    ZSECFORM5_NDTAB_dict['SWAP'] = to_bool(match.group('swap').strip())
            ...
            while key != 'END_DH':
                ...
                elif key == 'SWAP': ZSECFORM5_DTAB_dict['SWAP'] = to_bool(match.group('swap').strip())
